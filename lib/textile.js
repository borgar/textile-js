      ...
      if ( (m = re_block.exec( src )) ) {
        ...
        if ( (m = /\.(\.?)\s?/.exec( src )) ) {
          ...
          if ( block_type === 'bq' ) {
            ...
            list.add( [ 'pre', pba, ( sub_pba ? [ 'code', sub_pba, m1 ] : [ 'code', m1 ] ) ] );
          }
          else if ( block_type === 'notextile' ) {
            list.merge( parse_html( m1 ) );
          }
          else if ( block_type === '###' ) {
            // ignore the insides
          }
            ...
            pba['id'] = 'fn' + fnid;
            list.add( [ "p", pba, [ 'a', { 'href': '#fnr' + fnid }, [ 'sup', fnid ] ], ' ', m1 ] );
          }
          else { // heading | paragraph
            list.merge( paragraph( m1, block_type, pba, '\n' ) );
          }
          continue;
        }
        else {
