	...
	function parseDefList(src, options, charOffset, charPosToLine) {
	  ...
	  while (m = reItem.exec(src)) {
	    ...
	    while (terms.length) {
	      ...
	      deflist.push('\t', ['dt'].concat(addLineNumber({}, charPosToLine, charOffset, src.getPos() + localCharOffset), parsePhrase(term.trim(), options)), '\n');
	      if (options.showOriginalLineNumber) {
	    ...
	    deflist.push('\t', ['dd'].concat(addLineNumber({}, charPosToLine, charOffset, src.getPos() + localCharOffset), /=:$/.test(def) ? parseFlow(def.slice(0, -2).trim(), options, options.showOriginalLineNumber ? charPosToLine[(charOffset || 0) + localCharOffset + src.getPos()] : 0) : parsePhrase(def, options)), '\n');
	    src.advance(m[0]);
	  }
	  return deflist;
	}
