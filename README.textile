...
An HTML attribute @data-line@ is added to the HTML block elements. It's value is the line number in input text, of the first char of text inside that HTML block.|
|hooks|@[ function(jsonml :any[], param :any | undefined, Nodelevel :number) :any[], param? :any ]@|@[]@|Array of function (and optional initial parameter), applied in the order of the array to each JsonML node before rendering to HTML.
Each function returns the modified JsonML tree.|
